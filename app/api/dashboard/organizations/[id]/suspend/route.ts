    ...
    try {
        ...
        try {
            data = await apiFetch<{ message?: string }>(
                `/api/system-admin/organizations/${id}/suspend/`,
                {
                    method: "POST",
            ...
            // Fallback: if action endpoint is unavailable, suspend via status update.
            await apiFetch(`/api/system-admin/organizations/${id}/`, {
                method: "PATCH",
                json: { status: "suspended" },
            });
            data = { message: "Organization suspended successfully" };
        }
        ...
        return NextResponse.json(
            {
                message: message || "Failed to suspend organization",
                errors: data,
            },
            { status: status || 500 }
        );
